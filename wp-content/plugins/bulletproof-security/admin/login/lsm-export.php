	...
	if ( isset( $_POST['Submit-LSM-Export'] ) && current_user_can('manage_options') ) {
		...
		if ( file_exists($LSM_Master_CSV) ) {
			...
			if ( bps_Zip_LSM_Table_CSV() == true ) {
				...
				echo $bps_topDiv;
				$text = '<font color="green"><strong>'.__('The Login Security Table was exported successfully. Click the Download Zip Export button to download the Login Security Table lsm-master.zip file.', 'bulletproof-security').'</strong></font><br><div style="width:140px;font-size:1em;text-align:center;margin:10px 0px 0px 0px;padding:4px 6px 4px 6px;background-color:#e8e8e8;border:1px solid gray;"><a href="'.plugins_url( '/bulletproof-security/admin/login/lsm-master.zip' ).'" style="font-size:1em;font-weight:bold;text-decoration:none;">'.__('Download Zip Export', 'bulletproof-security').'</a></div>';
				echo $text;
				echo $bps_bottomDiv;
			}		
		}
