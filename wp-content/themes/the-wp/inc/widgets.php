	...
	function the_wp_enqueue_admin_widget_styles_scripts( $hook ) {
		...
		if ( 'widgets.php' == $hook || $widgetload ):
			...
			*/
			...
			wp_enqueue_style( 'the-wp-admin-widgets', CEEWP_THEMEURI . "/inc/admin-widgets.css", array(),  CEEWP_THEMEVER );
			/* Enqueue Scripts */
			wp_enqueue_script( 'the-wp-admin-widgets', CEEWP_THEMEURI . "/inc/admin-widgets.js", array( 'jquery' ), CEEWP_THEMEVER, true ); // Load in footer to maintain script heirarchy
		endif;
	}
...
abstract class The_WP_WP_Widget extends WP_Widget {
	...
	function render_field( $field, $value, $repeater = array() ){
		...
		switch( $type ) {
			...
			case 'separator' :
				?><div class="the-wp-widget-field-separator"></div><?php
				break;
